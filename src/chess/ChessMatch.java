...
public class ChessMatch {
    ...
    private Piece makeMove(Position source, Position target) {
        ...
        p.increaseMoveCount();
        Piece captiredPiece = board.removePiece(target);
        board.placePiece(p, target);

        if (captiredPiece != null) {
            piecesOnTheBoard.remove(captiredPiece);
            capturedPieces.add(captiredPiece);
        }
        ...
        if (p instanceof King && target.getCollumn() == source.getCollumn() + 2) {
            ...
            Position targeT = new Position(source.getRow(), source.getCollumn() + 1);
        ...
        if (p instanceof Pawn) {
            if (source.getCollumn() != target.getCollumn() && captiredPiece == null) {
                ...
                captiredPiece = board.removePiece(pawnPosition);
                capturedPieces.add(captiredPiece);
                piecesOnTheBoard.remove(captiredPiece);
            }
        }

        return captiredPiece;
